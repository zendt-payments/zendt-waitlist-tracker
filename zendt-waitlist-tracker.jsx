...
export default function ZendtWaitlistTracker() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const c = await window.storage.get(CONFIG_KEY, true);
        if (c && c.value) setConfig(JSON.parse(c.value));
      } catch (e) { /* first run */ }
      try {
        const d = await window.storage.get(DUPELOG_KEY, true);
  ...
  const saveConfig = async (next) => {
    setConfig(next);
    try { await window.storage.set(CONFIG_KEY, JSON.stringify(next), true); } catch (e) { /* ignore */ }
  };

  const logDupe = async (entry) => {
    const next = [{ ...entry, at: stamp() }, ...dupeLog].slice(0, 200);
    setDupeLog(next);
    try { await window.storage.set(DUPELOG_KEY, JSON.stringify(next), true); } catch (e) { /* ignore */ }
  };
